...
class GcpComputeInfoCollector(BaseHandler):
    ...
    def __init__(
        ...
        """
        Initializes the GCP Compute Info Collector with authenticated clients for
        various Compute Engine services. It expects the path to a service account
        ...
        try:
            ...
            logger.info("GCP Compute Engine clients initialized with custom service account credentials.")
            ...
            logger.error(
                f"Error initializing GCP Compute clients: {e}\n"
                "Please ensure the GCP_AGENT_CREDENTIALS environment variable points to a valid service account key file, "
                "and that the service account has necessary permissions to list and get Compute Engine resources."
            )
            raise
    ...
    def _get_iam_policy_compute_resource(self, client, project_id: str, zone_or_region: str, resource_name: str,
        ...
        try:
            if resource_type == "instance":
                policy = client.get_iam_policy(project=project_id, zone=zone_or_region, resource=resource_name)
            ...
            try:
                ...
                for instance in self.instances_client.list(request=request):
                    instance_details = {
                        ...
                        "iam_policy": self._get_iam_policy_compute_resource(
                ...
                logger.error(f"  Permission denied to list instances in zone '{zone}', project '{target_project_id}'. Error: {e}")
            except Exception as e:
                logger.error(f"  An unexpected error occurred while collecting instances in zone '{zone}'. Error: {e}", exc_info=True)
        else:
            logger.info(f"\n  Collecting VM instances in project '{target_project_id}' across all zones...")
            zones = await self._get_all_zones(target_project_id)
            for z in zones:
                instances_info.extend(await self.collect_instances(target_project_id, z)) # Recursive call with specific zone
    ...
    async def collect_disks(self, project_id: str = None, zone: str = None) -> list:
        ...
        if zone:
            ...
            try:
                ...
                logger.error(f"  Permission denied to list disks in zone '{zone}', project '{target_project_id}'. Error: {e}")
            except Exception as e:
                logger.error(f"  An unexpected error occurred while collecting disks in zone '{zone}'. Error: {e}", exc_info=True)
        else:
            logger.info(f"\n  Collecting persistent disks in project '{target_project_id}' across all zones...")
            zones = await self._get_all_zones(target_project_id)
            for z in zones:
    ...
    async def collect_subnetworks(self, project_id: str = None, region: str = None) -> list:
        ...
        if region:
            ...
            try:
                ...
                for subnetwork in self.networks_client.list_subnetworks(project=target_project_id, region=region):
                    subnetwork_details = {
                ...
                logger.error(f"  Permission denied to list subnetworks in region '{region}', project '{target_project_id}'. Error: {e}")
            except Exception as e:
                logger.error(f"  An unexpected error occurred while collecting subnetworks in region '{region}'. Error: {e}", exc_info=True)
        else:
    ...
    async def collect_firewall_rules(self, project_id: str = None) -> list:
        ...
        try:
            ...
            for firewall in self.firewalls_client.list(request=request):
                firewall_details = {
                    "name": firewall.name,
                    "id": firewall.id,
            ...
            logger.error(f"  An unexpected error occurred while collecting firewall rules. Error: {e}", exc_info=True)
    ...
    async def collect_images(self, project_id: str = None) -> list:
        ...
        try:
            ...
            for image in self.images_client.list(request=request):
                image_details = {
                    "name": image.name,
                    "id": image.id,
                    "status": image.status,
    ...
    async def collect_snapshots(self, project_id: str = None) -> list:
        ...
        try:
            ...
            for snapshot in self.snapshots_client.list(request=request):
                snapshot_details = {
                    "name": snapshot.name,
                    "id": snapshot.id,
    ...
    async def collect_all_compute_info(self, project_id: str = None) -> dict: # project_id is now optional
        ...
        try:
            ...
            for region in regions:
                all_compute_info["subnetworks"].extend(await self.collect_subnetworks(project_id=target_project_id, region=region))

            # Collect zonal resources (instances, disks)
            zones = await self._get_all_zones(target_project_id)
            for zone in zones:
                all_compute_info["instances"].extend(await self.collect_instances(project_id=target_project_id, zone=zone))
                all_compute_info["disks"].extend(await self.collect_disks(project_id=target_project_id, zone=zone))

        except Exception as e:
            logger.error(f"An unexpected error occurred during comprehensive Compute info collection: {e}", exc_info=True)
